...
import LLAL
from llal_loss import LossPredictionLoss
from acquistion import *
...
def train(hyp,  # path/to/hyp.yaml or hyp dictionary
    ...
    # Scheduler
    #print("####cos lr true or false: ", opt.cos_lr)
    if opt.cos_lr:
    ...
    print("#############ssssss lr###########: ",scheduler.get_last_lr()[0])
    ...
    train_loader, dataset = create_dataloader(train_path, imgsz, batch_size // WORLD_SIZE, gs, single_cls,
                                            ...
                                            prefix=colorstr('train: '), shuffle=True, training=True, subset=train_idx)
    ...
    compute_loss = ComputeLoss(model, False, batch_size)  # init loss class
    ...
    if method == "LLAL":
        ### Lossnet ###
        lossnet = LLAL.Lossnet().to(device)
        if opt.optimizer == "Adam":
            optimizer_lossnet = Adam(lossnet.parameters(), lr=hyp['lr0'], betas=(hyp['momentum'], 0.999))  # adjust beta1 to momentum
        else:
            optimizer_lossnet = SGD(lossnet.parameters(), lr=hyp['lr0'], momentum=hyp['momentum'], nesterov=True)
        #optimizer_lossnet = SGD(lossnet.parameters(), lr=0.01, momentum=0.9, weight_decay=5e-4)
        #scheduler_lossnet = lr_scheduler.MultiStepLR(optimizer_lossnet, milestones=[5, 10, 30])
        lossnet_scheduler = lr_scheduler.LambdaLR(optimizer_lossnet, lr_lambda=lf) 
                ...
                if AL == True:
                    if method == "LLAL":
                        ...
                        if epoch > LLAL_epoch_s:
                            ...
                            loss, loss_items, img_loss = compute_loss(pred, targets.to(device), use_LLAL=True)  # loss scaled by batch_size
                            ...
                            print("loss: ", loss)
                            loss = 0.5 * loss + 0.5 * loss_prediction_loss
                            #print("predict loss: ", loss_prediction_loss)
                        else:
                            pred = model(imgs)  # forward
                            loss, loss_items, img_loss = compute_loss(pred, targets.to(device), use_LLAL=False)

                    elif method == "Coreset" or method == "Entropy":
                            pred = model(imgs)  # forward
                            loss, loss_items, img_loss = compute_loss(pred, targets.to(device), use_LLAL=False)  # loss scaled by batch_size
    ...
    if AL == True:
        unlabel_loader = create_dataloader(train_path, imgsz, batch_size // WORLD_SIZE, gs, single_cls,
                                            ...
                                            rect=opt.rect, rank=LOCAL_RANK, workers=workers,
                                            image_weights=opt.image_weights, quad=opt.quad,
                                            prefix=colorstr('train: '), shuffle=True, training=True, subset=pool_idx)[0]
        ...
        if method == "LLAL":
            ...
            with torch.no_grad():
                lossnet.eval()
                pred_l = torch.tensor([]).to(device)
                for i, (imgs, targets, paths, _) in enumerate(unlabel_loader):
                    imgs = imgs.to(device, non_blocking=True).float() / 255.0  # uint8 to float32, 0-255 to 0.0-1.0
                    targets = targets.to(device)
                    pred = model(imgs)

                    #print("intermediate['first_fm']: ", intermediate['first_fm'].shape)
                    #print("intermediate['second_fm']: ", intermediate['second_fm'].shape)
                    #print("intermediate['third_fm']: ", intermediate['third_fm'].shape)
                    ...
                    loss_pred = lossnet(features)
                    loss_pred = loss_pred.view(loss_pred.size(0))
                    pred_l = torch.cat((pred_l, loss_pred))
            ...
            with torch.no_grad():
                for i, (im, targets, paths, _) in enumerate(unlabel_loader):
                    targets = targets.to(device)
                    im = im.to(device) 
                    ...
                    #print("train out shape: ", train_out[0].shape)
                    
                    # NMS
                    targets[:, 2:] *= torch.Tensor([width, height, width, height]).to(device)  # to pixels
                    lb = [targets[targets[:, 0] == i, 1:] for i in range(nb)] if save_hybrid1 else []  # for autolabelling
                    
                    #print("out before NMS entroyp: ", [len(a) for a in out])

                    out, entropy = non_max_suppression(out, conf_thres = 0.5, iou_thres = 0.6, labels=lb, multi_label=True, agnostic=single_cls, get_Entropy=True)
            ...
            #print("arg: ", arg)
            train_idx += list(torch.tensor(pool_idx)[arg][-count:].numpy()) 
